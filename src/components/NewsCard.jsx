...
export default function NewsCard({ article }) {
  ...
  return (
    <article key={article.id}
      className='mb-6 bg-white p-2 border-slate-300 border-2 rounded-lg shadow-lg hover:cursor-pointer hover:border-slate-500 transition ease-linear hover:translate-x-1'
    >
      <a href={article.url} target='_blank' className='flex flex-row gap-4'>
        ...
        <div className='p-4'>
          <h2 className='md:text-2xl text-lg font-medium mb-2'>{article.title}</h2>
          <p className='font-normal mb-4'>{article.summary}</p>
